def check_move(i: int, vals: list[list[int]]):
    for j in range(i+1, len(vals)-(i==-1)-1):
        if vals[i][0]+1<vals[i+1][0] or (vals[i][0]<vals[i+1][0] and vals[i][1]!=vals[i][1]):
            return True
    return False
...
def solve(rows:list[list[int]], cols:list[list[int]]):
    ...
    idl=(rows[1][0]-rows[0][0],abs(rows[0][1]-rows[1][1])) if len(rows)>1 else (0,0)
    # max element from right=iir1-iir0+abs(jir0-jir1)
    idr=(rows[-1][0]-rows[-2][0],abs(rows[-1][1]-rows[-2][1]))  if len(rows)>1 else (0,0)

    jdu=(cols[1][0]-cols[0][0],abs(cols[0][1]-cols[1][1]))  if len(cols)>1 else (0,0)
    jdd=(cols[-1][0]-cols[-2][0],abs(cols[-1][1]-cols[-2][1])) if len(cols)>1 else (0,0)
