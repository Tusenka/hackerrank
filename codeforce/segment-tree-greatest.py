...
class TreeNode:
    ...
    def load(self, a, i=0, j=-1):
        ...
        self.rmax_val = max(self.right.rmax_val, self.f(self.left.rmax_val, self.right.rmax_val))
        self.lmax_val = max(self.left.lmax_val, self.f(self.right.lmax_val, self.left.lmax_val))
    ...
    def get_at_least(self, k: int, l: int):
        if self.is_leaf:
            return self.range[0] if self.val>=k else -1

        if self.left.val>=k and self.left.range[0]>=l:
            return self.left.get_at_least(k, l)

        if self.right.val>=k and self.right.range[1]>=l:
            return self.right.get_at_least(k, l)
    ...
    def update(self, i, v):
        ...
        self.rmax_val = max(self.right.rmax_val, self.f(self.left.rmax_val, self.right.val))
        self.lmax_val = max(self.left.lmax_val, self.f(self.left.val, self.right.lmax_val))
...
for _ in range(m):
    ...
    if o[0]==1:
        ...
        print(tree.get_at_least(o[1], o[2]))
