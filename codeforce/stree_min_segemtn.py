...
class TreeNode:
    ...
    def find_min(self, l: int, r: int):
        ...
        if self.left_child._in_range(l,r) and self.right_child._in_range(l,r):
            return min(self.left_child.find_min(l,r), self.right_child.find_min(l, r))+self.added_value
    ...
    def _in_range(self, i, j):
        return self.range[0] <= i <= self.range[1] or self.range[0] <= j <= self.range[1]
