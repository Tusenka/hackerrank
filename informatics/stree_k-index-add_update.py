...
def update_nodes(res: list[list[int]], l: int, r: int, val, idx=0, begin=0, end=-1):
    ...
    lvalue = update_nodes(
        res=res, l=l, r=min(m, r), val=val, idx=idx * 2 + 1, begin=begin, end=m
    )
    rvalue = update_nodes(
        res=res, l=max(m + 1, l), r=r, val=val, idx=idx * 2 + 2, begin=m + 1, end=end
    )
...
def _get_value(res: list[list[int]], l: int, r: int, idx: int, begin: int, end: int):
    ...
    m = (begin + end) // 2
    return max(
        _get_value(res=res, l=l, r=r, idx=idx * 2 + 1, begin=begin, end=m),
        _get_value(res, l, r, idx * 2 + 2, m + 1, end),
    )
...
for i in range(t):
    ...
    if c[0] == "a":
        ...
        ans.append(get_value(res=res, l=int(c[1]) - 1, r=int(c[2]) - 1))
