...
class NTreeNode:
    p: NTreeNode | None
    val: int | None = -1
    values: dict[int, tuple[int, NTreeNode]] = dataclasses.field(default_factory=dict)
    level: int = 0
    children: list[NTreeNode] = dataclasses.field(default_factory=list)

    def _prepare(self, node: TreeNode, p: NTreeNode = None):
        self.val = node.val
        if p is None:
            self.p = self
        else:
...
def prepare(node: TreeNode, x: TreeNode, y: TreeNode, p: NTreeNode | None = None) -> tuple[NTreeNode, NTreeNode]:
    nnode = NTreeNode(p=p, val=node.val)
    if p is None:
        node.p.level = 0
    else:
    ...
    if node.left is not None:
        node.children.append(prepare(node, ))
...
n = int(input().rstrip())
arr = [NTreeNode(p=None) for i in range(n)]
root = 0
